import {ESX as Token, ESXElement} from '@es-esx/esx';
...
// generic utils
const asChildNodes = ({childNodes}: HTMLElement) => ({childNodes: [...childNodes]});
const getChild = ({childNodes}, i) => childNodes[i];
const getToken = ({children}, i) => children[i];
const invoke = ({value, properties, children}: ESXElement) => value(properties, ...children);
...
class View {
    ...
    constructor(token: Token, shouldParse = true) {
...
const isComponent = (e: ESXElement) => !!e.children[0]?.isDynamic
...
export default (options = {}) => {
    const document: Document = options.document || globalThis.document;
    ...
    const getComponentView = (view: View, component: ESXElement) => {
        view.dispose();
        view.dispose = effect(() => {
    ...
    const getNewView = (view: View, token: Token, createEffect) => {
        view.dispose();
        view = new View(token);
        if (createEffect)
    ...
    const getView = (view: View, token: ESXElement, createEffect) => isComponent(token) ?
